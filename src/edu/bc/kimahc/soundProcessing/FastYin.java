...
public final class FastYin {
	...
	public static final int DEFAULT_LOW_FREQ_CUTOFF = 50;
	public static final int DEFAULT_HIGH_FREQ_CUTOFF = 1200;
	/**
	...
	private int maxTau, minTau;
	public FastYin(final float audioSampleRate, final int bufferSize) {
		this(audioSampleRate, bufferSize, DEFAULT_THRESHOLD, DEFAULT_LOW_FREQ_CUTOFF, DEFAULT_HIGH_FREQ_CUTOFF);
	}
	 ...
	 */
	public FastYin(final float audioSampleRate, final int bufferSize, final double yinThreshold, final int lowFreqCutoff, final int highFreqCutoff) {
		...
		//fft = new FFT(bufferSize*2, audioSampleRate);
		fft = new FloatFFT(bufferSize);
		slowfft = new FFT(bufferSize*2,audioSampleRate);
		//result = new PitchDetectionResult();
		
		minTau = (int) (audioSampleRate/highFreqCutoff); //set highest hz to a specified cutoff (default of 1200)
		maxTau = (int) (audioSampleRate/lowFreqCutoff); //set lowest hz to a specified cutoff (default of 50)
	}
	...
	public float[] getDifference(final float[] audioBuffer) {
		// POWER TERM CALCULATION
		// ... for the power terms in equation (7) in the Yin paper
		float[] powerTerms = new float[yinBuffer.length];
		for (int j = 0; j < yinBuffer.length; ++j) {
		...
		//slowfft.forward(audioBufferFFT);
		fft.complexForward(audioBufferFFT);
	...
	private int absoluteThreshold() {
		...
		for (tau = minTau; tau < maxTau; tau++) { //search between min 50hz and max 1200 hz
		...
		if (tau == yinBuffer.length || yinBuffer[tau] >= threshold || probability > 1.0) {
			tau = -1;
			probability = 0;
			pitchSet = false;	
		} else {
